...
namespace Input {
    ...
    void Update() {
        ...
        if (KeyPressed(CV_KEY_ESCAPE)) {
            Back::ForceCloseWindow();
        }
        if (KeyPressed(CV_KEY_G)) {
            Back::ToggleFullscreen();
        }
        if (KeyPressed(CV_KEY_H)) {
	...
	int GetViewportMappedMouseX(int viewportWidth) {
		return (int)(_mouseX / Back::GetCurrentWindowWidth() * viewportWidth);
	}

	int GetViewportMappedMouseY(int viewportHeight) {
		return (int)(_mouseY / Back::GetCurrentWindowHeight() * viewportHeight);
	}
