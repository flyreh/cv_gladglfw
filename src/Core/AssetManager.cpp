...
namespace AssetManager {
  ...
  std::vector<Vertex> g_vertices; //cambiaer esto 
  std::vector<float> g_vertices2;
...
void AssetManager::CreateVertexData() {
    ...
    for (int i = 0; i < g_vertices.size(); i += 3) {
        Util::SetNormalAndTangentFromVertices(&g_vertices[i], &g_vertices[i + 1], &g_vertices[i + 2]);
		  //  std::cout << "TANGENTE [" << i << "]" << g_vertices[i].position.x << "," << g_vertices[i].position.y << "," << g_vertices[i].position.z << std::endl;
    }
...
void AssetManager::UploadVertexData() {
  ...
  // Creaci�n de VAO and VBO para cubo de luz (al ser un cubo, tiene los mismos vértices)

 
}
void AssetManager::UploadVertexLight() {

    GLBackVertex::UploadVertexLight(g_vertices);
}
