...
enum Alignment { CENTERED, TOP_LEFT, TOP_RIGHT, BOTTOM_LEFT, BOTTOM_RIGHT };
...
struct RenderItem3D {
  ...
  bool operator<(const RenderItem3D& obj) const { return meshIndex < obj.meshIndex; }
};
...
struct CSGVertex {
  ...
  CSGVertex(glm::vec3 pos) { position = pos; }
